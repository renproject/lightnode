...
func New(options Options, ctx context.Context, logger logrus.FieldLogger, sqlDB *sql.DB, client redis.Cmdable) Lightnode {
	...
	resolverI := resolver.New(options.Network, logger, cacher, multiStore, sqlDB, db, serverOptions, compatStore, versionStore, gpubkeyStore, bindings, verifier, options.ScreeningKey)
