	...
	Context("when verifying a lock tx", func() {
		It("should reject a lock tx with invalid gpubkey", func() {
			...
			test := func() bool {
				...
				bindings.HandleAccountLockInfo = func(ctx context.Context, chain multichain.Chain, asset multichain.Asset, txid pack.Bytes) (multichain.AccountTx, error) {
					pubKey := id.PubKey{}
					...
					return testutils.MockAccountTx{
						TxTo:    to,
						TxValue: amount,
					}, nil
				}
